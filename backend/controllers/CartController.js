      ...
      include: [
        {
          ...
          include: [
            {
              model: db.Product,
              as: "product", // phải đúng với as trong model prodetail.js
            },
            // Nếu muốn lấy thêm size, store thì include tiếp ở đây
            // { model: db.Size },
            // { model: db.Store }
          ],
        },
...
export async function insertCart(req, res) {
  ...
  // 1. Kiểm tra chỉ có session_id hoặc user_id (một trong hai)
  if ((session_id && user_id) || (!session_id && !user_id)) {
    return res.status(400).json({
      message:
        "Chỉ được cung cấp một trong hai: session_id hoặc user_id. Không được đồng thời hoặc để trống cả hai.",
    });
  }

  // 2. Kiểm tra trùng session_id hoặc user_id (nếu đã tồn tại)
  ...
  if (existingCart) {
    return res.status(409).json({
      message: session_id
        ? "Một giỏ hàng với session_id này đã tồn tại."
        : "Một giỏ hàng với user_id này đã tồn tại.",
    });
  }
