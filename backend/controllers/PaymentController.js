...
export async function createPayment(req, res) {// Nguyên lý hoạt động 
    ...
    try {
        ...
        const payment = await db.Payment.create({
            ...
            transaction_id: paymentResult.orderCode?.toString() || order.id.toString(),
            payment_url: paymentUrl,
            payos_order_code: payment_method === 'payos' ? paymentResult.orderCode : null,
...
export async function vnpayReturn(req, res) {
    try {
       ...
       if (!verification.isValid) {
            ...
            const redirectUrl = `${process.env.CLIENT_URL || 'http://localhost:5173'}/#payment-result?status=error&message=Invalid+signature&orderId=${orderId}`;
       ...
       if (!payment) {
            ...
            const redirectUrl = `${process.env.CLIENT_URL || 'http://localhost:5173'}/#payment-result?status=error&message=Payment+not+found&orderId=${orderId}`;
            return res.redirect(redirectUrl);
        }
        ...
        try {
            ...
            if(paymentStatus === 'completed' && payment.order.user_id){
                ...
                await transaction.commit();
               // ✅ Redirect về client với đúng format
           ...
           await transaction.rollback();
            ...
            const redirectUrl = `${process.env.CLIENT_URL || 'http://localhost:5173'}/#payment-result?status=error&message=Database+error&orderId=${orderId}`;
            return res.redirect(redirectUrl);
        }

    } catch (error) {
         console.error('❌ VNPAY return error:', error);
        return res.redirect(`${process.env.CLIENT_URL}/payment-result?status=error&message=${error.message}`);
    }
}
...
export async function verifyPayment(req, res) {
    try {
        ...
        const payosResult = await PayOSService.getPaymentLinkInformation(
            orderCode
        );
