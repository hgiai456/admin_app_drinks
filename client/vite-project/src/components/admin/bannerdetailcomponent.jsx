...
function BannerDetailComponent() {
    ...
    const fetchDetails = async () => {
        const data = await BannerDetailAPI.getAll();
        setDetails(data);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (editingId) {
            await BannerDetailAPI.update(editingId, detailObj);
        } else {
            await BannerDetailAPI.create(detailObj);
        }
    ...
    const handleDelete = async (id) => {
        await BannerDetailAPI.delete(id);
        fetchDetails();
    };
                    ...
                    {details.map((detail) => (
                        <tr key={detail.getId()}>
                            ...
                            <td>
                                ...
                                <button
                                    onClick={() => handleDelete(detail.getId())}
                                >
                                    Xóa
                                </button>
                            </td>
